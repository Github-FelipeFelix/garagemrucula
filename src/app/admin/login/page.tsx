...
export default function LoginPage() {
  ...
  // Erro devolvido pelo callback do OAuth (?erro=oauth).
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("erro") === "oauth") {
      setError("Não foi possível entrar com o Google. Confirme que a conta tem acesso ao painel.");
    }
  }, []);
    ...
    <div className="flex min-h-dvh items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-2xl border border-line bg-surface p-6">
        ...
        <button
          ...
          className="mt-6 flex w-full items-center justify-center gap-2 rounded-full border border-line bg-white px-6 py-3 text-base font-semibold text-black transition hover:bg-white/90 disabled:opacity-60"
        >
          <GoogleIcon />
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-3">
          ...
          <button type="submit" disabled={loading || googleLoading} className="btn btn-primary w-full">
