...
**🟢 SITE NO AR (domínio próprio!): https://www.garagemrucula.com.br** — apex `garagemrucula.com.br` redireciona (308) pro www; o `garagemrucula.vercel.app` também responde. Deploy automático a cada `git push`. Supabase com dados reais. v1 + cosméticos "UAU" + leva de melhorias (fotos HD, lightbox, filtros mobile) no ar.
...
**Estado (att. 01/07/2026, tarde) — domínio próprio + leva de melhorias + QA visual:**
...
1. 🚗 **Primo cadastrar os carros reais** + apagar o `fusca-teste`. Guia: `COMO-USAR-O-ADMIN.md`. (Obs.: o `fusca-teste` usa só o LOGO como foto — a qualidade nova das fotos só "aparece" com fotos reais de carro. O QR do admin agora sai com o domínio; se o Felipe viu `localhost`, era cache — recarregar com Ctrl+Shift+R.)
...
3. 💡 **Melhorias contínuas:** o Felipe deu mandato aberto ("quero algo incrível, os bugs a gente corrige"). Próximas ideias possíveis: zoom/pinch no Lightbox, toast de feedback ao salvar no admin, OG image por carro. Priorizar conforme ele for usando/pedindo.
